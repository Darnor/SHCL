...
int main() {
	...
	if (error) {
		...
		std::cout << "Error while creating info: " << shcl_error_message(error);
	...
	if (error) {
		std::cout << "Error while allocation input data memory: " << shcl_error_message(error);
		shcl_destroy_error(error);
		shcl_free(inData);
		shcl_destroy_info(info);
		return 1;
	...
	if (error) {
		std::cout << "Error while allocation output data memory: " << shcl_error_message(error);
		shcl_destroy_error(error);
		shcl_free(inData);
		shcl_free(outData);
		shcl_destroy_info(info);
	...
	for (int t { 0 }; t < numberOfImages; ++t) {
		for (int r { 0 }; r < rows; ++r) {
			for (int c { 0 }; c < columns; ++c) {
				...
				// Error check and cleanup if true.
				if (error) {
					std::cout << "Error while calculating image index: " << shcl_error_message(error);
					shcl_destroy_error(error);
					shcl_free(inData);
					shcl_free(outData);
					shcl_destroy_info(info);
				...
				inData[index][0] = static_cast<double>(c);
				inData[index][1] = 0;
			}
		}
	}
	...
	// Error check and cleanup if true.
	if (error) {
		std::cout << "Error while creating the shcl plan: " << shcl_error_message(error);
		shcl_destroy_error(error);
		shcl_free(inData);
		shcl_free(outData);
	...
	// Error check and cleanup if true.
	if (error) {
		std::cout << "Error while executing the shcl plan: " << shcl_error_message(error);
		shcl_destroy_error(error);
		shcl_free(inData);
		shcl_free(outData);
		shcl_destroy_plan(plan);
	...
	for (int i { 0 }; i < numberOfImages; ++i) {
		...
		for (int l { 0 }; l <= maxL; ++l) {
			...
			// Error check and cleanup if true.
			if (error) {
				std::cout << "Error while calculating the clm index: " << shcl_error_message(error);
				shcl_destroy_error(error);
				shcl_free(inData);
				shcl_free(outData);
				shcl_destroy_plan(plan);
			...
			std::cout << std::complex<double> { outData[index][0], outData[index][1] } << "\n";
		}
	}
